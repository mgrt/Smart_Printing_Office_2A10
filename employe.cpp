...
QSqlQueryModel *Employe::count(QString a,QString b)
{
    QSqlQueryModel * model=new QSqlQueryModel();
         model->setQuery("select email,mdp,role,nom from employee where email='"+a+"' and mdp='"+b+"'");
               setemail( model->data(model->index(0, 0)).toString());
                 setmdp( model->data(model->index(0, 1)).toString());
                  setrole(model->data(model->index(0, 2)).toString());
                  setnom(model->data(model->index(0, 3)).toString());
         return model ;
