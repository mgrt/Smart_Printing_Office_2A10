...
void MainWindow::update_label()
{
    ...
    if(data>="1")
     ...
     {   //ui->label->setText("on"); // si les données reçues de arduino via la liaison série sont égales à 1
    // alors afficher ON
        factures *ft= new factures();
        ft->test_poids("5");
        QString res=QString::number(ft->getMontant());
        QString resclt=QString::number(ft->getIdClientfact());


        int poids_tot=ft->getMontant()*ft->getIdClientfact();
        int minpoids, maxpoids;
        minpoids=poids_tot-3;
        QString minpoidsstr=QString::number(minpoids);
        maxpoids=poids_tot+3;
        QString maxpoidsstr=QString::number(maxpoids);
        if(data>minpoidsstr && data<maxpoidsstr)
        {
            ui->label->setText("bien");
        }
        else
        {
            ui->label->setText("attention quantite manquante");
        }
        ...
        ui->label->setText("stock vide poids =0");   // si les données reçues de arduino via la liaison série sont égales à 0
     //alors afficher ON
...
/*
void MainWindow::on_pbimp_4_clicked()
{
    ui->idS_2->setText("eeeeeeeeee");
      A.write_to_arduino("1");
}

void MainWindow::on_pb_on_clicked()
{
    ui->label->setText("eeeeeeeeee");
      A.write_to_arduino("1");
}*/

void MainWindow::on_pb_tester_clicked()
{
    A.write_to_arduino("1");
