...
QSqlQueryModel *factures::test_poids(QString id)
{
    QSqlQueryModel * model=new QSqlQueryModel();
    model->setQuery("select quantite, poids from stocks where id_produit like '"+id+"' ");
             setMontant(model->data(model->index(0, 0)).toInt());
             setIdClientfact(model->data(model->index(0, 1)).toInt());
    return model ;
}
